...
import { 
  Award, Sparkles, Gift, QrCode, History, User, 
  Copy, Check, Clock, RefreshCw, Camera, AlertCircle, CheckCircle2,
...
export default function CustomerDashboard({
  ...
  const [activeTab, setActiveTab] = useState<'home' | 'rewards' | 'code' | 'history' | 'profile'>('home');
  ...
  useEffect(() => {
    if (initialCouponCode) {
      ...
      if (matched) {
        ...
        const genericCodes = ['WELCOME50', 'CRM2026', 'KOFFEELOVER100', 'CHICSTYLE80'];
        if (genericCodes.includes(codeClean)) {
          setSuccessMessage(`พบรหัสสะสมแต้มแคมเปญ: ${codeClean} โปรดกดปุ่มยืนยันเพื่อรับคะแนนสะสม`);
  ...
  const loadData = () => {
    ...
    const currCust = allCustomers.find(c => c.id === currentCustomerId) || allCustomers[0];
    setCustomer(currCust);
    
    // Get approved shops
    const allShops = getShops().filter(s => s.registrationStatus === 'approved');
    setShops(allShops);
    ...
    setRewards(getRewards().filter(r => r.shopId === selectedShopId && r.isAvailable));
  ...
  if (!customer) return <div className="p-8 text-center">กำลังโหลดข้อมูลลูกค้า...</div>;
  ...
  const getTierThreshold = (tier: TierType) => {
    if (tier === 'Silver') return { next: 'Gold', target: 300, color: 'from-amber-600 to-amber-800' };
    if (tier === 'Gold') return { next: 'Platinum', target: 1000, color: 'from-yellow-500 to-amber-500' };
    return { next: 'Maxed', target: 1000, color: 'from-teal-400 to-cyan-500' };
  };

  const tierInfo = getTierThreshold(customer.tier);
  const pointsProgress = Math.min(100, (customer.currentPoints / tierInfo.target) * 100);
  ...
  const handlePromoSubmit = (e: React.FormEvent) => {
    ...
    if (matchedCoupon) {
      if (matchedCoupon.isUsed) {
        setErrorMessage('ลิงก์หรือรหัสรวบรวมแต้มนี้ถูกใช้ไปแล้ว คูปองสามารถสะสมสิทธิ์ได้ครั้งเดียว!');
        setTimeout(() => setErrorMessage(''), 3500);
        return;
      }
      if (new Date() > new Date(matchedCoupon.expiresAt)) {
        setErrorMessage('ลิงก์หรือรหัสสะสมแต้มพรีเมี่ยมนี้หมดอายุการใช้งานไปแล้ว');
        setTimeout(() => setErrorMessage(''), 3500);
        return;
      }
    ...
    setSuccessMessage(`ยินดีด้วย! คุณได้รับ +${pointsEarned} แต้มเรียบร้อยแล้ว`);
    setPromoCode('');
    onDataChange();
    loadData();
  ...
  const handleConfirmClaimDynamicCoupon = () => {
    ...
    setShowCouponConfirm(false);
    setSuccessMessage(`สะสมสิทธิ์สำเร็จ! ได้รับ +${pointsToAdd} แต้มจากกลุ่ม ${matched.shopName}`);
    setPromoCode('');
  ...
  // QR Simulator Scan
  const simulateScan = (points: number, shopName: string, shopId: string, desc: string) => {
    setScanning(true);
    setTimeout(() => {
      const allCustomers = getCustomers();
      const updatedCustomers = allCustomers.map(c => {
      ...
      setScanning(false);
      setSuccessMessage(`สแกนสำเร็จแล้ว! ได้รับ +${points} แต้ม จากร้าน ${shopName}`);
      onDataChange();
      loadData();
      setTimeout(() => setSuccessMessage(''), 4000);
    }, 1200);
  ...
  const handleConfirmRedeem = () => {
    ...
    setTimeout(() => {
      ...
      const updatedCustomers = allCustomers.map(c => {
        if (c.id === customer.id) {
          return { ...c, currentPoints: c.currentPoints - selectedReward.pointsCost };
        }
        return c;
      });
      saveCustomers(updatedCustomers);
  ...
  const handleUpdateProfile = (e: React.FormEvent) => {
    ...
    const updated = allCustomers.map(c => {
      if (c.id === customer.id) {
        return { ...c, name: profileName, phone: profilePhone, lineName: profileLineName };
      }
      return c;
    });
  ...
  return (
    <div className="relative min-h-[720px] bg-slate-50 font-sans text-slate-800 flex flex-col max-w-[420px] mx-auto sm:border-[8px] sm:border-slate-800 rounded-[40px] overflow-hidden shadow-[0_25px_60px_-15px_rgba(0,0,0,0.12)] border border-slate-200">
      ...
      <div className="bg-[#06C755] text-white px-4 py-3 flex items-center justify-between select-none shadow-xs border-b border-[#05b04b]">
        <div className="flex items-center gap-1.5 min-w-0">
          ...
          <span className="font-bold text-xs tracking-wide truncate">{activeShop?.name || 'CRM Line MiniApp'}</span>
        </div>
        <div className="flex items-center gap-3">
          ...
          <div className="relative">
            ...
            >
              {shops.map(s => (
                <option key={s.id} value={s.id} className="bg-white text-slate-800 text-xs">{s.name}</option>
              ))}
            </select>
            <div className="absolute right-1.5 top-1/2 -translate-y-1/2 pointer-events-none text-emerald-100 text-[8px] font-black">▼</div>
          </div>
          <span className="text-white hover:opacity-85 text-xs cursor-pointer font-bold select-none px-1">✕</span>
        </div>
      </div>
        ...
        {activeTab === 'home' && (
          <div className="space-y-4">
            ...
            <motion.div 
              ...
              style={{
                background: 'linear-gradient(135deg, #FFE8A3 0%, #F5CE62 30%, #DFAC4C 65%, #B8811E 100%)'
              }}
            >
              ...
              <div className="flex flex-col">
                <span className="text-[9px] tracking-wider uppercase opacity-75 text-amber-950 font-bold font-mono">Loyalty Privilege Club</span>
                <span className="text-base font-black tracking-tight text-amber-950 italic">
                  {customer.tier === 'Platinum' ? '⭐️ PLATINUM VIP MEMBER' : '👑 GOLD MEMBER CARD'}
                </span>
              </div>
              ...
              <div className="mt-8 flex justify-between items-end">
                <div className="space-y-0.5">
                  <p className="text-[9px] text-amber-900/80 font-bold">ชื่อสมาชิก / Member Name</p>
                  <p className="font-extrabold text-sm text-stone-900">{customer.name}</p>
                  <p className="text-[10px] font-mono text-amber-950/70 mt-1.5 font-bold tracking-widest">{customer.lineId.substring(0, 10).toUpperCase()}***</p>
                </div>
                ...
                >
                  ...
                  <span className="text-[9px] font-black text-amber-200">สะสมแต้ม</span>
                </button>
              </div>
              ...
              <div className="mt-4 pt-3 border-t border-amber-900/15 flex justify-between text-[11px] text-amber-950 font-bold">
                ...
                <span className="font-mono text-stone-950 font-extrabold text-xs">{customer.currentPoints} แต้ม (Points)</span>
              </div>
            </motion.div>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="bg-white border border-slate-200/60 rounded-2xl p-3.5 flex flex-col shadow-2xs">
                <span className="text-[10px] text-slate-455 font-bold">แต้มใช้แลกรางวัล</span>
                <span className="text-xl font-extrabold text-amber-600 font-mono mt-0.5">{customer.currentPoints} <span className="text-xs font-semibold text-slate-400">แต้ม</span></span>
              </div>
              <div className="bg-white border border-slate-200/60 rounded-2xl p-3.5 flex flex-col shadow-2xs">
                <span className="text-[10px] text-slate-455 font-bold">แต้มสะสมทั้งหมด</span>
                <span className="text-xl font-extrabold text-slate-800 font-mono mt-0.5">{customer.lifetimePoints} <span className="text-xs font-semibold text-slate-400">แต้ม</span></span>
              </div>
            </div>
            ...
            <div className="bg-white border border-slate-200/60 rounded-2xl p-4 space-y-2.5 shadow-2xs">
              <div className="flex justify-between items-center text-xs">
                <div className="flex items-center gap-1.5">
                  ...
                  <span className="font-extrabold text-slate-800">สถานะ VIP ระดับล่าสุด</span>
                </div>
                <span className="text-[10px] text-slate-500 font-medium">
                  {customer.tier === 'Platinum' ? 'สิทธิ์ประโยชน์ Platinum ครบครัน' : `สะสมอีก ${tierInfo.target - customer.currentPoints} เพื่ออัปเกรด`}
                </span>
              </div>
              ...
              <div className="w-full bg-slate-100 rounded-full h-2">
                <div 
                  ...
                  style={{ width: `${customer.tier === 'Platinum' ? 100 : pointsProgress}%` }}
                />
              </div>
            ...
            <div className="flex justify-between items-center pt-2">
              <h3 className="text-xs font-extrabold text-slate-800 uppercase tracking-widest">โปรโมชั่นและสิทธิพิเศษ</h3>
              <span className="text-[10px] text-amber-600 font-bold flex items-center gap-0.5 cursor-pointer hover:underline" onClick={() => setActiveTab('rewards')}>
                ...
                return (
                  ...
                  >
                    <div className="h-28 overflow-hidden relative">
                      ...
                      />
                      <span className={`absolute top-2 right-2 text-[8px] font-black uppercase px-2.5 py-1 rounded-full text-white shadow-sm ${ban.isAd ? 'bg-amber-500 text-stone-950' : 'bg-[#06C755]'}`}>
                        {ban.isAd ? 'ADVERTISEMENT' : 'PROMOTION'}
                      </span>
                    </div>
                    <div className="p-3.5 space-y-1">
                      <h4 className="text-xs font-extrabold text-slate-900">{ban.title}</h4>
                      <p className="text-[10.5px] text-slate-500 line-clamp-2 leading-relaxed">{ban.description}</p>
                      <div className="flex justify-between items-center pt-1 text-[9px] text-slate-400 font-mono font-semibold">
                        ...
                        {ban.isAd && (
                          <span className="text-amber-600 underline">ดูรายละเอียดเพิ่มเติม</span>
                        )}
                      </div>
                    </div>
                  </div>
        ...
        {activeTab === 'rewards' && (
          <div className="space-y-4">
            <div className="space-y-1">
              ...
              <p className="text-[10px] text-slate-500 font-medium font-sans">เลือกของรางวัลสุดพิเศษเพื่อรวบรวมรหัสไปเพื่อยืนยันที่หน้าร้านได้ทันที</p>
            </div>
            ...
            <div className="bg-white border border-slate-200/60 rounded-2xl p-3 flex items-center gap-3 shadow-2xs">
              <img src={activeShop?.logo} className="w-10 h-10 rounded-full object-cover border border-slate-100" referrerPolicy="no-referrer" />
              <div>
                <p className="text-xs font-bold text-slate-900">{activeShop?.name}</p>
                <p className="text-[10px] text-slate-500 font-semibold">{activeShop?.category} • สะสม 10 บ. = 1 แต้ม</p>
              </div>
            </div>
                ...
                return (
                  ...
                  >
                    <div className="h-24 relative overflow-hidden">
                      <img src={rew.image} alt={rew.name} className="w-full h-full object-cover" referrerPolicy="no-referrer" />
                      ...
                      <div className={`absolute bottom-2 right-2 text-[8px] font-black px-2.5 py-0.5 rounded-full shadow-sm text-white ${canRedeem ? 'bg-emerald-600' : 'bg-slate-500/80'}`}>
                        {canRedeem ? 'พร้อมแลก' : 'แต้มไม่พอ'}
                      </div>
                    </div>
                    <div className="p-2.5 flex-1 flex flex-col justify-between">
                      <div className="space-y-0.5">
                        <h4 className="text-[11px] font-extrabold text-slate-900 line-clamp-1">{rew.name}</h4>
                        <p className="text-[9.5px] text-slate-500 line-clamp-2 leading-relaxed">{rew.description}</p>
                      </div>
                      <div className="flex justify-between items-center text-[9px] text-slate-400 font-mono font-bold mt-1 pt-1.5 border-t border-slate-100">
                        ...
                        <span className="text-amber-600 font-black">กดแลกรางวัล</span>
                      </div>
                    </div>
                  </div>
                );
            ...
            {rewards.length === 0 && (
              <div className="p-8 text-center text-slate-400 border border-dashed border-slate-250 rounded-2xl bg-white space-y-2.5 shadow-2xs">
                ...
                <p className="text-xs font-semibold">ขณะนี้ยังไม่มีของรางวัลเปิดให้บริการสำหรับร้านหมวดนี้</p>
              </div>
            )}
          </div>
        )}
        ...
        {activeTab === 'code' && (
          <div className="space-y-4">
            <div className="space-y-1">
              ...
              <p className="text-[10px] text-slate-500 font-medium">กรณีได้รับโค้ดจัดกิจกรรมจากร้านค้า หรือ สแกนคิวอาร์โค้ดสิทธิพิเศษ</p>
            </div>

            {/* Manual Promo code entry */}
            <form onSubmit={handlePromoSubmit} className="bg-white border border-slate-200/80 p-4.5 rounded-3xl space-y-3.5 shadow-xs">
              <div className="space-y-1.5">
                <label className="text-[11px] font-bold text-slate-650">กรอกรหัสสะสมแต้ม (สลักตัวอักษรพิมพ์ใหญ่)</label>
              ...
              <div className="bg-slate-50/80 rounded-2xl p-3 space-y-2 text-[10px] border border-slate-200/60 shadow-inner">
                <p className="text-amber-700 font-bold">รหัสสะสมแต้มสาธารณะเปิดซิมจำลอง:</p>
                <div className="grid grid-cols-2 gap-2 font-mono text-slate-500 font-semibold">
                  <div className="flex justify-between hover:text-slate-900 cursor-pointer pr-1 transition" onClick={() => setPromoCode('WELCOME50')}>
                    <span>• WELCOME50</span>
                    <span className="text-emerald-600 font-bold text-[9px]">(+50)</span>
                  </div>
                  <div className="flex justify-between hover:text-slate-900 cursor-pointer pr-1 transition" onClick={() => setPromoCode('CRM2026')}>
                    <span>• CRM2026</span>
                    <span className="text-emerald-600 font-bold text-[9px]">(+150)</span>
                  </div>
                  <div className="flex justify-between hover:text-slate-900 cursor-pointer pr-1 transition" onClick={() => setPromoCode('KOFFEELOVER100')}>
                    <span>• KOFFEELOVER100</span>
                    <span className="text-emerald-600 font-bold text-[9px]">(+100)</span>
                  </div>
                </div>
              </div>
            </form>
            ...
            <div className="bg-white border border-slate-200/80 p-4.5 rounded-3xl space-y-3.5 shadow-xs">
              <div className="flex justify-between items-center">
                <h4 className="text-xs font-extrabold text-slate-800">สแกนคิวอาร์โค้ดรับแต้มหน้าร้าน</h4>
              ...
              <div className="relative border-2 border-dashed border-slate-200 rounded-2xl bg-slate-50 aspect-video flex flex-col justify-center items-center overflow-hidden p-4 text-center shadow-inner">
                ...
                {scanning ? (
                  <div className="space-y-3">
                    ...
                    <p className="text-[11px] font-mono font-bold text-slate-500">กำลังสแกนและยืนยันความปลอดภัยเข้ารับแต้ม...</p>
                  </div>
                ) : (
                  <div className="space-y-2">
                    ...
                    <p className="text-xs font-semibold text-slate-500">กดปุ่มจำลองการแสกนคิวอาร์รับแต้มด้านล่าง</p>
                  </div>
                )}
              ...
              <div className="space-y-2.5">
                <span className="text-[9px] uppercase font-mono text-slate-400 font-bold block">จำลองรหัสสแกนคิวอาร์รับแต้มพิเศษ:</span>
                <div className="grid grid-cols-2 gap-2">
                  <button 
                    type="button"
                    onClick={() => simulateScan(50, activeShop?.name || 'ร้านค้าพาร์ทเนอร์', activeShop?.id || 'shop-1', 'สแกนด่วนกิจกรรมเช้า')} 
                    disabled={scanning}
                  ...
                  <button 
                    type="button"
                    onClick={() => simulateScan(100, activeShop?.name || 'ร้านค้าพาร์ทเนอร์', activeShop?.id || 'shop-1', 'โปรด่วนพิเศษซื้อครบ 500')} 
                    disabled={scanning}
        ...
        {activeTab === 'history' && (
          <div className="space-y-4">
            <div className="space-y-1">
              ...
              <p className="text-[10px] text-slate-500 font-medium font-sans">ติดตามการใช้งานและสถานะคูปองของรางวัลได้ตลอด 24 ชั่วโมง</p>
            </div>
            ...
            <div className="space-y-3">
              {transactions
                .filter(t => (historySubTab === 'earn' ? t.type === 'earn' : t.type === 'redeem'))
                .map(t => (
                  <div key={t.id} className="bg-white border border-slate-200/60 rounded-2xl p-3.5 space-y-2 shadow-2xs">
                    <div className="flex justify-between items-start text-xs">
                      <div>
                        <span className="font-extrabold text-slate-900">{t.shopName}</span>
                        <div className="flex items-center gap-1.5 text-[9px] text-slate-400 font-mono mt-0.5">
                          <Clock className="w-3 h-3 text-slate-400" />
                          <span>{new Date(t.createdAt).toLocaleDateString('th-TH', { hour: 'numeric', minute: 'numeric' })}</span>
                        </div>
                      </div>
                      
                      {/* Point numeric visual */}
                      <span className={`font-mono font-black text-sm ${t.type === 'earn' ? 'text-emerald-600' : 'text-rose-650'}`}>
                        {t.type === 'earn' ? `+${t.points}` : `-${t.points}`}
                      </span>
                    </div>

                    <p className="text-[10.5px] text-slate-500 font-medium font-sans leading-relaxed">{t.description}</p>
                        ...
                        <div className="flex items-center gap-1">
                          {t.status === 'pending' && (
                            <>
                              ...
                              <span className="text-amber-600 font-bold uppercase">รออนุมัติหน้าร้าน</span>
                            </>
                          )}
                          {t.status === 'completed' && (
                            <>
                              <span className="w-1.5 h-1.5 bg-emerald-500 rounded-full" />
                              <span className="text-emerald-600 font-bold uppercase">อนุมัติแล้ว</span>
                            </>
                          )}
                          {t.status === 'rejected' && (
                            <>
                              <span className="w-1.5 h-1.5 bg-rose-500 rounded-full" />
                              <span className="text-rose-500 font-bold uppercase text-decoration-line: line-through">ปฏิเสธ / ยกเลิก</span>
                            </>
                          )}
                        </div>
                      </div>
              ...
              {transactions.filter(t => (historySubTab === 'earn' ? t.type === 'earn' : t.type === 'redeem')).length === 0 && (
                <div className="p-8 text-center text-slate-400 border border-dashed border-slate-200 rounded-2xl bg-white shadow-2xs">
                  ...
                  <p className="text-xs mt-2 font-semibold">ยังไม่พบรายการกิจกรรมของคุณในหน้านี้</p>
                </div>
              )}
            </div>
        ...
        {activeTab === 'profile' && (
          <div className="space-y-4">
            <div className="space-y-1">
              ...
              <p className="text-[10px] text-slate-500 font-medium">จัดการข้อมูลประจําตัวและช่องทางเพื่อรับข่าวสารพิเศษเฉพาะคนพิเศษ</p>
            </div>

            {/* Profile editing form */}
            <form onSubmit={handleUpdateProfile} className="bg-white border border-slate-200/80 p-4.5 rounded-3xl space-y-4 shadow-xs">
              <div className="flex items-center gap-3.5 pb-3.5 border-b border-slate-100">
                <img src={customer.avatar} className="w-14 h-14 rounded-full object-cover border border-slate-100 shadow-sm" referrerPolicy="no-referrer" />
                <div>
                  <h4 className="text-xs font-extrabold text-slate-900">{customer.lineName}</h4>
                  <p className="text-[9px] text-slate-400 font-mono font-semibold">LINE UUID: U11aa2...26ff</p>
              ...
              <div className="space-y-3.5">
                <div className="space-y-1">
                  <label className="text-[10px] text-slate-500 font-bold">ชื่อจริง-นามสกุล :</label>
                  <input 
                    type="text"
                    value={profileName}
                ...
                <div className="space-y-1">
                  <label className="text-[10px] text-slate-500 font-bold">เบอร์โทรศัพท์มือถือ :</label>
                  <input 
                    type="tel"
                    value={profilePhone}
                ...
                <div className="space-y-1">
                  <label className="text-[10px] text-slate-500 font-bold">ชื่อไลน์ (LINE Name) :</label>
                  <input 
                    type="text"
            ...
            <div className="bg-amber-50/50 border border-amber-200/65 p-4 rounded-2xl space-y-2.5 text-[10.5px] shadow-2xs">
              <span className="text-[11px] font-extrabold text-amber-900">สิทธิพิเศษประจำระดับ {customer.tier} :</span>
              <ul className="space-y-1.5 text-stone-700 list-disc list-inside font-semibold font-sans">
                ...
                <li>สิทธิ์ในการเข้าร่วมปาร์ตี้กาแฟเวิร์กชอปกราบนมัสการปีละ 2 ครั้ง</li>
                {customer.tier === 'Platinum' ? (
                  <>
                    <li className="text-amber-700 font-bold">ส่วนลดวันเกิดทันที 20% และเค้กวันเกิดจานพิเศษฟรี</li>
                    <li className="text-amber-700 font-bold">สิทธิ์แลกของขวัญจำกัดสต็อกล่วงหน้าก่อนสมาชิกปกติ 7 วัน</li>
                  </>
                ) : (
      ...
      <div className="absolute bottom-0 left-0 right-0 h-16 bg-white/95 backdrop-blur-md border-t border-slate-200/85 flex justify-around items-center select-none z-10 shadow-lg">
        ...
        >
          ...
          <span className="text-[9px] font-bold font-sans text-emerald-600">ใส่โค้ด</span>
        </button>
        ...
        {showQrModal && (
          ...
          >
            ...
            >
              ...
              <div className="space-y-1">
                <h4 className="text-sm font-extrabold text-amber-650 tracking-wider">คิวอาร์สะสมแต้มของหนู</h4>
                <p className="text-[10.5px] text-slate-500 font-semibold font-sans">ยื่นให้เจ้าหน้าที่หน้าร้านเพื่อสะสมแต้มพรีเมี่ยมได้ทันที</p>
              </div>
              ...
              <div className="bg-slate-50 rounded-2xl p-3 border border-slate-100 space-y-1 shadow-inner">
                <p className="text-[10px] text-slate-400 font-mono font-bold">รหัสคิวอาร์ส่วนตัวสมาชิก</p>
                <p className="text-xs font-bold font-mono tracking-widest text-slate-800">
                  {customer.phone.replace(/-/g, '')}-{customer.lineId.substring(1, 5).toUpperCase()}
                </p>
              </div>
          ...
          >
            ...
            >
              <div className="flex justify-between items-center pb-2.5 border-b border-slate-100">
                <span className="text-xs font-black text-slate-400 uppercase tracking-widest">รายละเอียดสิทธิ์แลกรางวัล</span>
                <button 
                  type="button"
              ...
              <div className="space-y-4">
                <img src={selectedReward.image} alt={selectedReward.name} className="w-full h-32 object-cover rounded-2xl border border-slate-100" referrerPolicy="no-referrer" />
                
                <div className="space-y-1">
                  <div className="flex justify-between items-start">
                    <h4 className="text-xs font-extrabold text-slate-900">{selectedReward.name}</h4>
                  ...
                  <p className="text-[10px] text-slate-500 leading-relaxed pt-1 font-semibold font-sans">{selectedReward.description}</p>
                </div>
                ...
                {!isRedeemSuccess ? (
                  <div className="pt-2">
                    {customer.currentPoints >= selectedReward.pointsCost ? (
                      ...
                      >
                        {isRedeeming ? 'กำลังตรวจสอบสิทธิ์...' : 'ยืนยันเพื่อ แลกรับของรางวัล'}
                      </button>
                    ) : (
                  ...
                  <div className="bg-emerald-50 border border-emerald-100 rounded-2xl p-4.5 text-center space-y-3">
                    ...
                    <div className="space-y-1">
                      <h5 className="text-xs font-black text-emerald-600">ยื่นคำขอแลกสำเร็จแล้ว!</h5>
                      <p className="text-[10px] font-semibold text-slate-500 leading-relaxed px-5 font-sans">กรุณาแจ้งเบอร์โทรศัพท์หรือเปิดหน้าประวัติรับคูปอง เพื่อให้แคชเชียร์สาขากดยืนยันทางระบบ</p>
                    </div>
                    <button 
                      type="button"
                      onClick={() => {
          ...
          >
            ...
            >
              ...
              <div className="pt-6 space-y-1">
                ...
                <h4 className="text-base font-black text-slate-900">ยืนยันรับแต้มจากหน้าร้านค้า</h4>
                <p className="text-xs text-slate-500 font-bold font-sans">✨ {pendingCoupon.shopName}</p>
              </div>

              {/* Points display card */}
              <div className="bg-emerald-50/55 rounded-2xl p-4 border border-emerald-100 space-y-1 shadow-inner">
                <span className="text-[10px] text-slate-500 block font-bold">จำนวนแต้มที่คุณจะได้รับ</span>
                <span className="text-3xl font-black font-mono text-emerald-600 block">
                  +{pendingCoupon.points} <span className="text-xs font-bold">แต้ม</span>
                </span>
                <p className="text-[10px] text-slate-500 font-semibold font-sans">รายละเอียดแคมเปญ: {pendingCoupon.description}</p>
              </div>
              ...
              <div className="text-left bg-slate-50 p-3.5 rounded-2xl border border-slate-150 text-[10px] space-y-1 text-slate-500 font-mono font-bold">
                <div className="flex justify-between">
                  <span>รหัสคูปอง:</span>
                  <span className="text-slate-800 font-black">{pendingCoupon.code}</span>
                </div>
                <div className="flex justify-between">
                  <span>เวลาหมดอายุ:</span>
                  <span className="text-amber-750 font-black">
                    {new Date(pendingCoupon.expiresAt).toLocaleTimeString('th-TH', { hour: '2-digit', minute: '2-digit' })} น.
                  </span>
                </div>
                <div className="flex justify-between">
                  <span>เงื่อนไขความปลอดภัย:</span>
                  <span className="text-emerald-600">สะสมคะแนนได้เพียง 1 ครั้งเท่านั้น</span>
                </div>
              </div>
