...
import { 
  Sparkles, AppWindow, Store, ShieldAlert, Award, 
  Layers, HelpCircle, Gift, Users, Activity, CheckCircle, Shield
} from 'lucide-react';

export default function App() {
  const [activeRole, setActiveRole] = useState<'customer' | 'owner' | 'webmaster'>('customer');
  ...
  const allShopsCount = getShops().length;
  const pendingClaimsCount = getTransactions().filter(t => t.status === 'pending' && t.type === 'redeem').length;
  const pendingShopsCount = getShops().filter(s => s.registrationStatus === 'pending').length;

  return (
    <div className="min-h-screen bg-slate-50 text-slate-800 flex flex-col font-sans select-none antialiased">
      
      {/* 🚀 Sticky Global Pilot Control Deck */}
      <div className="bg-white/90 backdrop-blur-md border-b border-slate-200/80 px-4 py-3 sticky top-0 z-50 shadow-sm">
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row md:items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h1 className="text-xs sm:text-sm font-extrabold text-slate-900 tracking-tight">ระบบสะสมแต้ม Loyalty Program MiniApp สลับสิทธิ์</h1>
              <span className="text-[10px] text-slate-500 font-mono font-medium">Platform Integration Prototype (LINE OA & Browser Multi-User)</span>
            </div>
          </div>
          ...
          <div className="flex bg-slate-100 p-1 rounded-2xl border border-slate-200/80 gap-1 select-none">
            ...
            >
              <span className={`w-1.5 h-1.5 rounded-full ${activeRole === 'customer' ? 'bg-white animate-pulse' : 'bg-emerald-500'}`} />
      ...
      <div className="bg-white border-b border-slate-200/60 py-2.5 px-4 text-xs text-slate-600 shadow-2xs">
        <div className="max-w-7xl mx-auto flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2">
          ...
          <div className="flex items-center gap-1.5 text-slate-650">
            ...
            {activeRole === 'customer' && (
              <span>💡 **หน้าสมาร์ทโฟนจําลองลูกค้า**: คุณเข้าใช้งานในฐานะลูกค้า VIP ของแบรนด์ ลองกด **"แลกรางวัล"** หรือสแกนเพื่อรับแต้ม และลองไปกดอนุมัติที่หลังบ้านร้านค้าได้เลย!</span>
            )}
            {activeRole === 'owner' && (
              <span>💡 **หน้าร้านค้าแอดมิน**: ควบคุมสาขา จัดระเบียบแต้ม กด **อนุมัติบิลการแลกสินค้า** ด้านบน หรือคลิกสร้างคิวอาร์แจกแต้ม แล้วลองเข้าไปดูผลลัพธ์หน้ามือถือลูกค้า!</span>
            )}
            {activeRole === 'webmaster' && (
              <span>💡 **หน้าเว็ปบอร์ดแอดมิน**: ตรวจสอบคำขอร่วมแฟรนไชส์แบรนด์และยอดข้อมูลแพลตฟอร์ม CRM ทั้งระบบ สมัครร้านค้าเสร็จจะไปปรากฏให้เปิดสะสมแต้ม!</span>
            )}
          </div>
      ...
      <main className="flex-1 max-w-7xl mx-auto w-full p-4 sm:p-6 md:p-8 flex flex-col justify-center">
        ...
        {activeRole === 'customer' && (
          <div className="py-4 flex flex-col justify-center items-center">
            ...
            <div className="text-center mb-4 space-y-1">
              <span className="text-[10px] tracking-wider uppercase opacity-40 font-mono text-slate-500 font-bold">Mobile Simulated Device Iframe Layout</span>
              <p className="text-xs text-slate-500">หน้าจอนี้ปรับขนาดแสดงผลสัดส่วนเหมาะสมกับโมบายล์สกรีน LINE LIFF</p>
            </div>
      ...
      <footer className="bg-white border-t border-slate-200/80 py-5 px-6 text-center text-xs text-slate-500 shadow-2xs mt-auto">
        <p>© 2026 CRM Customer Loyalty Platform. โครงร่างสร้างเว็บแอพสะสมแต้มพรีเมี่ยมยุคใหม่</p>
      </footer>
